...
public class StringCalculatorApplication {
	...
	private StringCalculatorApplication(String delimiter, String numbers) {
	...
	private static StringCalculatorApplication parseInput(String input) {
		if (input.startsWith("//")) {
			String[] headerAndNumberSequence = input.split("\n", 2);
			String delimiter = parseDelimiter(headerAndNumberSequence[0]);
			return new StringCalculatorApplication(delimiter, headerAndNumberSequence[1]);
		} else {
			return new StringCalculatorApplication(",|\n", input);
		}
	}
